...
At the accepted SHA, first read pinned `manifest.json` and pinned `VERSION` and require version agreement. Then load exactly `manifest.startup_slice`; Berta2 declares only self-contained `entry/STARTUP.md`. HELP follows `manifest.help`; EXECUTING follows `manifest.entrypoint` and `manifest.core[]`, or a bundle verified to contain those members in order. The descriptor is a later execution/release description, not navigation authority.

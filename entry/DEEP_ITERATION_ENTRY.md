...
- D: when enabled/useful, run non-local intervention sessions. Proposals may evolve until Decree. D result must reintegrate into Main with concrete consequence.
...
The model chooses when and how to use these mechanisms beyond frozen minimums. The runtime validates lifecycle/reference/timing facts, not intellectual merit.

## Completion, recovery and finalization

Keep completion gaps revisioned and reopenable. Meeting minima is necessary where applicable but never itself a quality stop command. Before delivery, close or consciously retain non-blocking gaps, perform a semantic completion assessment, finalize timing, verify clock/ledger parity and workspace integrity, then persist final summary.
...
Return the real task result first, followed only by the compact canonical proof. Do not expose hidden chain-of-thought or turn the workspace into a user-facing protocol report unless the task itself asks for audit details.

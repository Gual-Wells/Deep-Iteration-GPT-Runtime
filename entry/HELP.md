...
缺省处理先区分“整套全缺省”和“局部缺省”：

1. 若调用头完全没有参数 token，也没有 `S` / `D` / `L` 标记（包括 `DIGR：任务`、`DIGR()：任务` 及中文别名等价形式），直接固定为：
   `DIGR(3,3min,5,1,S(3,1min,5,1),D(3),L(1))`。
2. 只要显式给出任意参数或标记，就进入局部缺省规则：先应用 `B=0`、`b=0`、`L(1)`；
3. 再由原生模型结合 U0 与全部已给参数/标记，对缺失的 `N/T/R/n/t/r/s` 做语义补全；
4. 形成并冻结本轮 Effective Contract。冻结的是本轮承诺，不冻结策略。

因此，全缺省调用不会再按任务规模漂移：其 N/T/R/B/S/D/L 全部固定为上述整套配置。局部缺省则仍保留原有相对补全能力。例如：

- 显式 `T>0` 而省略 B：B 按局部缺省固定为 0，T 是 soft target；
- 显式 `B=1` 而省略 T：T 仍必须按任务规模语义补全，补全结果成为 hard lower bound；
- `t/b` 同理；
- 裸数字永远不能被猜成 T/t 的时间单位；
- 显式 `S`、`D` 或 `L` 标记即表示不是“整套全缺省”。
...
| 参数 | 语义 | 整套全缺省 | 局部省略时 |
|---|---|---|---|
| `N` | MAIN 最少有效进化次数 | `3` | 语义补全 |
| `T` | Formal Active Task Time 目标 | `3min` | 语义补全 |
| `R` | MAIN 候选结果最少整体重入次数 | `5` | 语义补全 |
| `B` | T 时间政策：0 soft / 1 hard | `1` | `0` |
| `S(n,t,r,b)` | 来源进化下限 / 来源有效时间目标 / 来源重入下限 / t 时间政策 | `S(3,1min,5,1)` | n/t/r 语义补全，b=`0` |
| `D(s)` | 最少完成并重新整合的 D intervention 数 | `D(3)` | 语义补全 |
| `L(e)` | D 隔离实现目标，`e∈{1,2,3}` | `L(1)` | `L(1)` |
...
`S`、`S()`、`D`、`D()`、`L`、`L()` 都是合法标记。`S()` 表示 n/t/r 留给语义补全且 b=0；`D()` 表示 s 留给语义补全；`L()` 表示 L1。由于这些都是显式标记，它们不会触发整套全缺省合同。
...
EXECUTING 调用先建立可信 Clock Genesis，再做参数解析、U0 冻结与合同形成。Effective Contract 包含显式参数、整套全缺省合同或局部缺省/语义补全结果、SourceDisposition、L 目标及用户硬约束。

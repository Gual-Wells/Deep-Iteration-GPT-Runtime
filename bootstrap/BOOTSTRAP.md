...
For manifests declaring `startup_slice`, acquire only that slice before repository classification. For this Alpha 3 commit the slice is `bootstrap/BOOTSTRAP.md` plus `entry/STARTUP.md`. Do not pre-load the full core before deciding NATIVE/HELP/INVALID/EXECUTING.
...
Legacy pinned manifests without `startup_slice` follow their own declared navigation and are not retrofitted with Alpha 3 startup semantics.

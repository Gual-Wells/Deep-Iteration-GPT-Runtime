# DIGR Transparent Machine Index — Alpha 9
...
- execution transport: pre-Genesis exact runtime package carrying `execution_bundle.path`;
...
After task work begins, repository/runtime processes and external tools may live in different host domains. Alpha 9 uses persisted work leases so known MAIN/SOURCE/D_EXCLUSIVE work can cross a verified same-boot boundary without losing formal time. An unleased formal boundary becomes an explicit coverage gap; it is never silently deleted or guessed into a state.

### Implementation delivery reality

Repository visibility and executor availability are separate capabilities. If the host cannot natively move exact same-SHA files into an executor, use manifest.runtime_distribution when declared. Alpha 9 may provide a durable same-commit GitHub prerelease asset and a transient same-commit Actions artifact; either remains byte-delivery transport only, and every helper member must still verify against the pinned Git tree before use.
...
After this index is understood, continue the remaining manifest-declared `startup_slice` in order. For this pinned Alpha 9 structure that means `bootstrap/BOOTSTRAP.md` then `entry/STARTUP.md`.

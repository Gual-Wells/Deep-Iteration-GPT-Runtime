...
class TestRepoContract(unittest.TestCase):
    def test_version_manifest_and_corrected_interfaces(self):
        self.assertEqual((ROOT/'VERSION').read_text().strip(),'5.0.0-alpha.2');m=json.loads((ROOT/'manifest.json').read_text());self.assertEqual(m['version'],'5.0.0-alpha.2');self.assertEqual(m['protocol'],'digr-v5.0');self.assertEqual(m['workspace_spec'],'workspace/layout-v2.json');self.assertEqual(m['clock_journal_schema'],1);self.assertEqual((m['routing_schema'],m['run_session_schema'],m['workspace_schema'],m['event_receipt_schema']),(2,2,2,2));self.assertEqual(m['invocation_surface_schema'],2);self.assertEqual(m['parameter_resolution_schema'],1)

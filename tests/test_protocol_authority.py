...
SHA='a'*40
MANIFEST=b'{"protocol":"digr-v5.0","version":"5.0.0-alpha.5"}'
...
class TestProtocolAuthority(unittest.TestCase):
    def test_frozen_run_identity_bound_to_route(self):
        p=ProtocolIdentity('digr-v5.0','5.0.0-alpha.5',REPO,SHA)
        a=ProtocolAuthority(route(),p)
        self.assertEqual(a.P_run.version,'5.0.0-alpha.5')
    ...
    def test_authority_requires_manifest_and_VERSION_binding(self):
        ...
        self.assertEqual(a.P_run.protocol,'digr-v5.0'); self.assertEqual(a.P_run.version,'5.0.0-alpha.5')
        badv=b'4.1.0\n'
    ...
    def test_full_commit_and_exact_repo_required(self):
        with self.assertRaises(ValueError): ProtocolIdentity('digr-v5.0','5.0.0-alpha.5',REPO,'stable')
        with self.assertRaises(ValueError): ProtocolIdentity('digr-v5.0','5.0.0-alpha.5','Other/Repo',SHA)
        with self.assertRaises(ValueError): ProtocolIdentity('','5.0.0-alpha.5',REPO,SHA)

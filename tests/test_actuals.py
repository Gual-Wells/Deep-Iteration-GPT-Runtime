...
class TestActuals(unittest.TestCase):
    def start(self,td,contract,msg='DIGR：x'):
        c=FakeClock();r=LiveDIGRRun.start(authority(),msg,Path(td),c,run_id='digr-12345678');r.resolve_parameters();r.freeze_u0('x');r.freeze_contract(contract);r.transition(WorkState.MAIN,c());r.save_strategy(StrategyState(0,'model','route'));return r,c

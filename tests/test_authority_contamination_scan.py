...
class TestAuthorityContaminationScan(unittest.TestCase):
    def test_local_router_has_transport_not_execution_semantics(self):
        ...
        for x in ('候选路由','Gual-Wells/Deep-Iteration-GPT-Runtime','/git/ref/heads/stable','/branches/stable','raw.githubusercontent.com/Gual-Wells/Deep-Iteration-GPT-Runtime/{SHA}/{PATH}','manifest.json','VERSION','startup_slice','entrypoint','core','不得用聊天上下文','没有尝试本身不是路由失败'):self.assertIn(x,text)
